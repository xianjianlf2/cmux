...
final class SidebarScrollerVisibilityController {
    ...
    private weak var scroller: SidebarVisibilityScroller?
    ...
    func synchronizeScroller() {
        ...
        let nextScroller: SidebarVisibilityScroller
        if let installedScroller = scrollView.verticalScroller as? SidebarVisibilityScroller {
            nextScroller = installedScroller
        } else {
            let replacement = SidebarVisibilityScroller(frame: scrollView.verticalScroller?.frame ?? .zero)
            replacement.controlSize = scrollView.verticalScroller?.controlSize ?? .regular
            replacement.knobStyle = scrollView.verticalScroller?.knobStyle ?? .default
            scrollView.verticalScroller = replacement
            nextScroller = replacement
        }
        guard scroller !== nextScroller else { return }

        scroller = nextScroller
        nextScroller.alphaValue = 0
        nextScroller.setUserScrollVisible(false)
    }
    ...
    private func showScroller() {
        ...
        fadeGeneration &+= 1
        scroller.setUserScrollVisible(true)
    ...
    private func fadeScroller() {
        ...
        NSAnimationContext.runAnimationGroup { context in
            ...
            Task { @MainActor [weak self, weak scroller] in
                ...
                scroller?.setUserScrollVisible(false)
            }
        }
    }
}

/// Prevents AppKit's preference-driven layout passes from making the sidebar
/// knob visible while idle. The controller is the sole owner of visibility.
@MainActor
final class SidebarVisibilityScroller: NSScroller {
    private var isUserScrollVisible = false

    override class var isCompatibleWithOverlayScrollers: Bool { true }

    override var isHidden: Bool {
        get { super.isHidden }
        set { super.isHidden = !isUserScrollVisible }
    }

    func setUserScrollVisible(_ visible: Bool) {
        isUserScrollVisible = visible
