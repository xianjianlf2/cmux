enum SidebarScrollIndicatorVisibilityEvent {
    case willStartLiveScroll
    case didLiveScroll
    case didEndLiveScroll
}

enum SidebarScrollIndicatorVisibilityAction: Equatable {
    case show
    case fade
    case showThenFade
}
...
/// never arrive.
struct SidebarScrollIndicatorVisibilityState {
    private var isLiveScrollBracketed = false

    mutating func handle(_ event: SidebarScrollIndicatorVisibilityEvent) -> SidebarScrollIndicatorVisibilityAction {
        switch event {
        case .willStartLiveScroll:
